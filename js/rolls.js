...
function doReset() {
  ...
  // ── Vyčisti všetky aplikované achievement rewards ──
  // Každý reward sa resetuje na default (null / default hodnotu)
  const achivKeysToRemove = [
    'dnd_active_title',
    'dnd_active_aura',
    'dnd_active_flavor',
    'dnd_active_stat_color',
    'dnd_name_style',
    'dnd_gem_style',
    'dnd_ds_skin',
    'dnd_header_deco',
    'dnd_card_border',
    'dnd_showcase_achievements',
    'dnd_tracker_counts'
  ];
  achivKeysToRemove.forEach(k => localStorage.removeItem(k));

  // Vizuálne odober aplikované efekty bez reloadu
  try {
    // Aura (canvas + body classes)
    if (typeof renderPortraitAura === 'function') renderPortraitAura();
    // Portrait rank frame
    if (typeof updatePortraitRankFromAchievements === 'function') updatePortraitRankFromAchievements();
    // Stat box colors → reset na default
    if (typeof applyStatBoxColors === 'function') applyStatBoxColors();
    // Name style → default
    if (typeof applyNameStyle === 'function') applyNameStyle('ns_default');
    // Gem style → default
    if (typeof applyGemStyle === 'function') applyGemStyle('gem_default');
    // Death save skin → default
    if (typeof applyDsSkin === 'function') applyDsSkin('ds_default');
    // Header deco → default
    if (typeof applyHeaderDeco === 'function') applyHeaderDeco('hd_default');
    // Card border → default
    if (typeof applyCardBorder === 'function') applyCardBorder('cb_default');
    // Portrait flavor text → skryť
    const flavorEl = document.getElementById('portraitFlavorText');
    if (flavorEl) { flavorEl.textContent = ''; flavorEl.style.display = 'none'; }
    // Active title v headeri / portrait → odober
    if (typeof renderPortraitTitle === 'function') renderPortraitTitle();
  } catch(e) { /* ak niektorá funkcia neexistuje, ignoruj */ }
