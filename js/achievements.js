...
// ═══════════════════════════════════════════
//  RESET ALL ACHIEVEMENTS + REWARDS
// ═══════════════════════════════════════════

function confirmResetAchievements() {
  if (!confirm('Reset all achievements and trackers? This will also remove all applied visual rewards.')) return;

  // 1. Vymaž achievements a trackery
  achvUnlocked = {};
  saveAchievements();
  trackerCounts = {};
  saveTrackerCounts();

  // 2. Vymaž všetky reward localStorage kľúče
  var rewardKeys = [
    'dnd_active_title',
    'dnd_active_aura',
    'dnd_active_flavor',
    ...
    'dnd_pinned_achievements'
  ];
  rewardKeys.forEach(function(k) { localStorage.removeItem(k); });

  // 3. Vizuálne resetuj každý reward späť na default
  try { applyNameStyle('ns_default'); }         catch(e) {}
  try { applyGemStyle('gem_default'); }         catch(e) {}
  try { applyDsSkin('ds_default'); }            catch(e) {}
  try { applyHeaderDeco('hd_default'); }        catch(e) {}
  try { applyCardBorder('cb_default'); }        catch(e) {}
  try { applyStatBoxColors(); }                 catch(e) {}
  try { renderPortraitAura(); }                 catch(e) {}
  try { updatePortraitRank(); }                 catch(e) {}

  // Flavor text
  var flavorEl = document.getElementById('portraitFlavorText');
  if (flavorEl) { flavorEl.textContent = ''; flavorEl.style.display = 'none'; }

  // 4. Prerendruj UI
  try { renderAchievements(); }                 catch(e) {}
  try { renderRewardsPanel(); }                 catch(e) {}
  try { renderTitleBadge(); }                   catch(e) {}
  try { renderPriorityHud(); }                  catch(e) {}
  try { updatePriorityHudBadge(); }             catch(e) {}

  if (typeof showToast === 'function') showToast('Achievements reset ✦');
}
