...
function loadData() {
  // Migrate legacy single-character save to slot 1 (one-time migration)
  const legacy = localStorage.getItem('dnd5e_chronicle');
  if (legacy && !localStorage.getItem(slotKey(1))) {
    localStorage.setItem(slotKey(1), legacy);
  }

  const slot = getActiveSlot();
  const raw = localStorage.getItem(slotKey(slot)) || legacy;
  if (raw) {
    try {
  ...
  // Render switcher after a tick (DOM must be ready)
  setTimeout(renderCharacterSwitcher, 0);
}

// ═══════════════════════════════════════════
//  MULTI-CHARACTER SLOT SYSTEM (up to 10)
// ═══════════════════════════════════════════

const MAX_SLOTS = 10;

function getActiveSlot() {
  return parseInt(localStorage.getItem('dnd5e_active_slot') || '1', 10);
}

function setActiveSlot(n) {
  localStorage.setItem('dnd5e_active_slot', String(n));
}

function slotKey(n) {
  return 'dnd5e_chronicle_' + n;
}

function getSlotPreview(n) {
  // Returns {name, class, level} or null
  const raw = localStorage.getItem(slotKey(n));
  if (!raw) return null;
  try {
    const d = JSON.parse(raw);
    return { name: d.charName || '', cls: d.charClass || '', level: d.charLevel || 1 };
  } catch(e) { return null; }
}
...
function switchToSlot(n) {
  ...
  if (raw) {
    try {
      ...
      // Run same migrations as loadData
      if (typeof data.generalNotes === 'string') {
        const old = data.generalNotes;
        data.generalNotes = old.trim() ? [{ title:'Old Notes', category:'Other', content:old, tags:'', _open:true }] : [];
      }
  ...
  initAll();
  renderCharacterSwitcher();
  const name = data.charName || ('Slot ' + n);
  showToast('Switched to: ' + name + ' ✦');
}

function renderCharacterSwitcher() {
  const container = document.getElementById('charSwitcherSlots');
  if (!container) return;
  ...
  for (let i = 1; i <= MAX_SLOTS; i++) {
    ...
    const isEmpty = !preview || !preview.name;
    const label = isEmpty ? 'Empty' : preview.name;
    const sublabel = isEmpty ? ('Slot ' + i) : ((preview.cls || '?') + ' · Lv' + (preview.level || 1));
    html += `<div class="cs-slot${isActive ? ' cs-slot-active' : ''}${isEmpty ? ' cs-slot-empty' : ''}" onclick="switchToSlot(${i})" title="Switch to slot ${i}">
      <div class="cs-slot-num">${i}</div>
      <div class="cs-slot-info">
        <div class="cs-slot-name">${label}</div>
        <div class="cs-slot-sub">${sublabel}</div>
      </div>
    </div>`;
  }
  container.innerHTML = html;
  // Update the toggle button label
  const labelEl = document.getElementById('charManagerActiveLabel');
  if (labelEl) {
    const preview = getSlotPreview(active);
    const name = (preview && preview.name) ? preview.name : ('Slot ' + active);
    labelEl.textContent = name;
  }
}

function toggleCharManager() {
  const panel = document.getElementById('charManagerPanel');
  if (!panel) return;
  ...
  if (isOpen) {
    ...
    setTimeout(() => {
      document.addEventListener('click', _charManagerOutsideClick, { once: true });
    }, 0);
  }
}
...
function _charManagerOutsideClick(e) {
  const wrap = document.getElementById('charManagerWrap');
...
function showAboutModal() {
  ...
  div.innerHTML = `
    <div class="about-modal">
      ...
      <div class="about-dragon">⚔</div>
      <div class="about-title">CHRONICLE</div>
      <div class="about-subtitle">D&amp;D 5th Edition · Character Sheet</div>
      <div class="about-version">Version 1.0.0</div>

      <div class="about-divider"></div>

      <div class="about-credit">Created by</div>
      <div class="about-author">Anima</div>
      <div class="about-tagline">Built with ❤️ for the D&amp;D community</div>

      <div class="about-divider"></div>

      <div class="about-links">
        <a class="about-link about-link-discord" href="https://discord.com/users/anima1887" target="_blank">
          <svg class="about-link-icon" viewBox="0 0 24 24" fill="currentColor"><path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03z"/></svg>
          <span>Discord</span>
          <span class="about-link-handle">anima1887</span>
        </a>
        <a class="about-link about-link-instagram" href="https://www.instagram.com/animatroniofry/" target="_blank">
          <svg class="about-link-icon" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z"/></svg>
          <span>Instagram</span>
          <span class="about-link-handle">@animatroniofry</span>
        </a>
        <a class="about-link about-link-kofi" href="https://ko-fi.com/F1F31Z21R2" target="_blank">
          <svg class="about-link-icon" viewBox="0 0 24 24" fill="currentColor"><path d="M23.881 8.948c-.773-4.085-4.859-4.593-4.859-4.593H.723c-.604 0-.679.798-.679.798s-.082 7.324-.022 11.822c.164 2.424 2.586 2.672 2.586 2.672s8.267-.023 11.966-.049c2.438-.426 2.683-2.566 2.658-3.734 4.352.24 7.422-2.831 6.649-6.916zm-11.062 3.511c-1.246 1.453-4.011 3.976-4.011 3.976s-.121.119-.31.023c-.076-.057-.108-.09-.108-.09-.443-.441-3.368-3.049-4.034-3.954-.709-.965-1.041-2.7-.091-3.71.951-1.01 3.005-1.086 4.363.407 0 0 1.565-1.782 3.468-.963 1.904.82 1.832 3.011.723 4.311zm6.173.478c-.928.116-1.682.028-1.682.028V7.284h1.77s1.971.551 1.971 2.638c0 1.913-.985 2.667-2.059 3.015z"/></svg>
          <span>Ko-fi</span>
          <span class="about-link-handle">Support the dev ☕</span>
        </a>
      </div>

      <div class="about-footer">© 2025 Anima · Free to use &amp; share</div>
    </div>
  `;
  document.body.appendChild(div);
  requestAnimationFrame(() => div.classList.add('open'));
  div.addEventListener('click', e => { if (e.target === div) closeAboutModal(); });
}
